...
from .SolidWorksDialogHandler import SolidWorksDialogHandler, SolidWorksReaderWizard # @UnresolvedImport
from .CuraPreferences import PreferencesAdvanced
...
class SolidWorksReader(CommonCOMReader):
    def __init__(self):
        ...
        if DEBUG:
            ...
            error_message = Message(i18n_catalog.i18nc("@info:status", "RUNNING IN DEBUG MODE!\nIT IS INTENDED THAT THE PLUGIN WILL MISSBEHAVE!!!"))
            error_message.setTitle("SolidWorks plugin - DEBUG MODE")
            error_message.show()
        ...
        # Initializing setting storage
        self.preferences = PreferencesAdvanced("cura_solidworks")

        # General settings
        self.preferences.addPreference("checks_at_initialization", True)

        # conversion settings
        self.preferences.addPreference("preferred_installation", -1)
        self.preferences.addPreference("export_quality", 10)
        self.preferences.addPreference("auto_rotate", True)

        # UI settings
        self.preferences.addPreference("show_export_settings_always", True)
        ...
        self._dialog_handler = SolidWorksDialogHandler(self)
    ...
    def checksAtInitialization(self):
        return self.preferences.getValue("checks_at_initialization")
    ...
    def _prefered_app_name(self):
        installation_code = self.preferences.getValue("preferred_installation")
    ...
    def isServiceRegistered(self, major_version):
        service_name = self.getVersionedServiceName(major_version)
        try:
    ...
    def getOpenCommandOfWindowsService(self, service_name):
        registry_entry = "{}\\shell\\open\\command".format(service_name)
        open_command = winreg.QueryValue(winreg.HKEY_CLASSES_ROOT, registry_entry)
        Logger.log("d", "The open command for {} is: {}".format(service_name, open_command))
        return open_command

    def getPathToExecutableByAPInumber(self, major_version):
        executable_extension = ".exe"
        service_name = self.getVersionedServiceName(major_version)
        path_to_executable = self.getOpenCommandOfWindowsService(service_name)
        path_to_executable = path_to_executable[:path_to_executable.find(executable_extension)+len(executable_extension)+1]
        path_to_executable = convertDosPathIntoLongPath(path_to_executable)
        return path_to_executable

    def getSoftwareInstallPath(self, major_version):
        path_to_executable = self.getPathToExecutableByAPInumber(major_version)
        path_to_installation = os.path.split(path_to_executable)[0]
        return path_to_installation
    ...
    def isSoftwareInstallPath(self, major_version):
        ...
        try:
            ...
            Logger.logException("e", "Could not determine installation path of SolidWorks API={}".format(major_version))
            return False

        if major_version in SolidWorkVersions.major_version_name.keys():
            installation_name =  SolidWorkVersions.major_version_name[major_version]
        else:
            installation_name =  "SolidWorks API={}".format(major_version)

        Logger.log("d", "Found {} installed at: {}".format(installation_name, repr(installation_path)))
        return True
    ...
    def isVersionOperational(self, version):
        ...
        if DEBUG:
            if EMULATE_VERSION_API is version:
                Logger.log("d", "DEBUG: Passing all tests for API={}!".format(version))
    ...
    def preStartApp(self, options):
        options["app_export_quality"] = self.preferences.getValue("export_quality")
        options["app_auto_rotate"] = self.preferences.getValue("auto_rotate")
